  ...
  implements Testlet
{
  public void test(TestHarness harness)
  {
    ...
    // Check #1.
    oldText = UIManager.get("AbstractUndoableEdit.redoText");
    try
      {
        UIManager.put("AbstractUndoableEdit.redoText", "Fooredo");
        harness.check(mgr.getRedoPresentationName(), "Fooredo");
      }
    finally
      {
    ...
    // Check #2.
    mgr.undo();
    harness.check(mgr.getRedoPresentationName(), "RedoPres2");

    // Check #3.
    mgr.undo();
    harness.check(mgr.getRedoPresentationName(), "RedoPres1");
