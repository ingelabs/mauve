...
/**
 * Checks whether the CubicCurve2D.Float.getBounds2D method works
 * correctly.
 *
  ...
  implements Testlet
{
  public void test(TestHarness harness)
  {
    ...
    curve = new CubicCurve2D.Float(1, -2, -3, 4, 5, 7, -8, 9);
    bounds = curve.getBounds2D();
    harness.check(bounds instanceof Rectangle2D.Float);  // 1
    harness.check(bounds.getX(), -8);                    // 2
    harness.check(bounds.getY(), -2);                    // 3
    harness.check(bounds.getWidth(), 13);                // 4
    harness.check(bounds.getHeight(), 11);               // 5
  }
}
