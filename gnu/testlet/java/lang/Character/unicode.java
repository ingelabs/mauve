...
public class unicode implements Testlet
{
  ...
  public void performTests()
  {
	...
	if ((// "Ll".equals(chars[i].category) &&
	     ...
	     chars[i].lowercase == 0 &&
	     (chars[i].uppercase != 0 ||
	      (chars[i].name.indexOf("SMALL LETTER") != -1) ||
	      (chars[i].name.indexOf("SMALL LIGATURE") != -1))) !=
	...
	if ( // "Lu".equals(chars[i].category)
	    ((i < 0x2000 || i > 0x2fff) &&
	      chars[i].uppercase == 0 &&
	      (chars[i].lowercase != 0 ||
	       (chars[i].name.indexOf("CAPITAL LETTER") != -1) ||
	       (chars[i].name.indexOf("CAPITAL LIGATURE") != -1))) !=
	...
	if ((range (i, 0x0030, 0x0039) ||
             ...
             range (i, 0xff10, 0xff19)) != Character.isDigit((char) i))
	  {
	    reportError(i,
	...
	if (chars[i].category.charAt(0) == 'Z' && i != 0x00a0 && i != 0xFEFF)
	  t = true;
	...
	if (((chars[i].category.charAt(0) == 'L') ||
	    ...
	    Character.isJavaIdentifierStart(i))
	  {
	    reportError(i,
			Character.isJavaIdentifierStart(i) ?
			"javaindetifierstart" : "not-javaidentfierstart");
	  }
	else checkPassed();
  ...
  public void test(TestHarness harness)
  {
    try {
      ...
      harness.debug("Benchmark : load:" + (midtime-start) + 
		    "ms   tests:" +  (System.currentTimeMillis() - midtime) +
		    "ms");
    }
    catch (Exception e)
      {
